...
export function ProductByAttributeSection({ section, language }: HomepageSectionProps) {
  const settings = section.settings || {};
  const maxItems = settings.maxItems || 8;
  const columns = settings.columns || 4;
  const attributeKey = section.attribute_key || "";
  const attributeValue = section.attribute_value || "";

  // Build filter based on attribute
  const filters: Record<string, string | number | boolean> = {
    language,
  };

  if (attributeKey && attributeValue) {
    // Convert attribute key to the expected filter format (e.g., attr_category)
    const filterKey = attributeKey.startsWith("attr_")
      ? attributeKey
      : `attr_${attributeKey}`;
    filters[filterKey] = attributeValue;
  }
  ...
  const viewAllLink =
    ...
    (attributeKey && attributeValue
      ? `/products?${attributeKey.startsWith("attr_") ? attributeKey : `attr_${attributeKey}`}=${encodeURIComponent(attributeValue)}`
      : "/products");
